/**
 * CineChroma — app.js  v5.0 (Cosmos.so Aesthetic Engine)
 * Strict Black & White · Centered 2D Color Picker Modal
 * Burger Drawer · Staggered Masonry Grid Layout
 */
...
const I18N = {
  fr: {
    search_placeholder:  'Essayez un titre, un réalisateur, une couleur…',
    search:              'Rechercher',
    ...
    genres:              'Genres cinématographiques',
  ...
  en: {
    search_placeholder:  'Try a title, director, color…',
    search:              'Search',
...
function syncUIFromState() {
  ...
  dom.drawerTolValue.textContent = state.colorThreshold;
...
/* ============================================================
   ACTIVE FILTER BAR (COSMOS HEADER BELOW APP HEADER)
============================================================ */
function renderActiveFilterBar() {
  const hasColors = state.activeColors.length > 0;
  const hasGenre = Boolean(state.activeGenre);

  if (!hasColors && !hasGenre && !state.searchQuery) {
  ...
  if (hasColors) {
    ...
    dom.filterHeadingText.textContent = state.activeGenre === '__favorites__' ? t('favorites') : state.activeGenre;
...
function openColorModal() {
  ...
  // Set initial color (red or active color)
...
/* ============================================================
   BURGER DRAWER (FILTERS, GENRES & SORT)
============================================================ */
function buildDrawerGenreChips() {
  ...
  favChip.className = `drawer-chip drawer-chip--fav${state.activeGenre === '__favorites__' ? ' active' : ''}`;
  favChip.textContent = t('favorites');
  favChip.addEventListener('click', () => selectGenre('__favorites__', favChip));
  dom.drawerGenreChips.appendChild(favChip);

  for (const g of genres) {
    const chip = document.createElement('button');
    chip.className = `drawer-chip${state.activeGenre === g ? ' active' : ''}`;
    chip.textContent = g;
...
function selectGenre(genre, chipEl) {
  if (state.activeGenre === genre) {
    ...
    $$('.drawer-chip.active').forEach(c => c.classList.remove('active'));
...
function applyFiltersAndRender() {
  ...
  if (state.activeGenre === '__favorites__') {
...
/* ============================================================
   COSMOS STAGGERED MASONRY GRID RENDERING
============================================================ */
function renderGrid(reset = false) {
  ...
  page.forEach((film, index) => {
    // Assign varied aspect ratio class for Cosmos visual rhythm
...
/* ============================================================
   CARD BUILDER (Cosmos Staggered & Rounded)
============================================================ */
...
function toggleFavorite(filmId, btnEl) {
  ...
  buildDrawerGenreChips();
  if (state.activeGenre === '__favorites__') applyFiltersAndRender();
}
...
function populateModal(film) {
  ...
  dom.modalGenres.innerHTML = (film.genres || []).map(g => `<span class="genre-tag">${esc(g)}</span>`).join('');
  ...
  $('.summary-tab[data-lang="fr"]').classList.add('active');
...
function updateURL() {
  ...
  if (state.activeGenre)                       p.set('genre', state.activeGenre);
...
function readURL() {
  ...
  if (p.has('genre'))  state.activeGenre = p.get('genre');
