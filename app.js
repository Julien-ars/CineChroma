...
function syncUIFromState() {
  ...
  // Show active genres count badge on filters button
  if (state.activeGenres.size > 0) {
    dom.activeFilterBadge.hidden = false;
    dom.activeFilterBadge.textContent = state.activeGenres.size;
  } else {
    dom.activeFilterBadge.hidden = true;
  }

  // Clear selections button inside sub-navbar
  dom.clearAllColorsBtn.hidden = (state.activeColors.length === 0 && state.activeGenres.size === 0);
...
function startHeroGlowCycle() {
  ...
  _heroGlowInterval = setInterval(() => {
    ...
    if (dom.heroAmbientGlow && !dom.landingHero.hasAttribute('hidden')) {
      dom.heroAmbientGlow.style.background = `radial-gradient(circle, ${hexToRgba(colors[idx], 0.15)} 0%, rgba(0,0,0,0) 70%)`;
    }
  }, 5000);
}
